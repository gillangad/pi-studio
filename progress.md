...
- 2026-05-10: Followed up on a master-session regression where opening the transcript also rendered a second composer. `ChatView` can now render transcript-only mode, so the master popup shows replies and tool calls without duplicating the master bar input.
